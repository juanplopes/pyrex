...
A maior parte das mudanças aconteceu para permiti-las alcançar um poder de expressão maior. Entretanto, esta conveniência trouxe o custo de tornar o \emph{match} um problema mais difícil computacionalmente do que a teoria descreve. Na maior parte das linguagens modernas o \emph{match} de \emph{regexes} (como são comumente conhecidas) é um problema NP-completo. Além disso, por particularidades nas implementações, mesmo expressões que poderiam ser reconhecidas em tempo polinomial são resolvidas com soluções exponenciais. 
...
Tal poder expressivo tem seu preço. Match de backreferences é um problema NP-completo, logo, os melhores algoritmos conhecidos ainda tem pior caso exponencial. É possível, porém, escrever algoritmos polinomiais para expressões que não fazem uso de backreferences.
...
\section{Match de backreferences: um problema NP-completo}
...
\subsection{MATCH em NP-completo}

Para provar que um problema pertence ao conjunto NP-completo, uma das formas é provar é garantindo que ele faz parte do conjunto NP e do conjunto NP-difícil.

Normalmente, a primeira parte (provar pertinência em NP) é mais fácil, consiste em mostrar que dada uma eviência é possível verificá-la em tempo polinomial.

A segunda parte pode ser um pouco mais complicada. Geralmente envolve mostrar que é possível reduzir um problema NP-díficil ao problema que se quer provar em tempo polinomial, e.g. provando que é possível reduzir 3SAT ao problema em questão em tempo polinomial.

Para fins de referência, definiremos o problema de reconhecimento de cadeias utilizando expressões regulares com backreferences como $MATCH(E, S)$ onde $E$ é uma cadeia que define uma expressão regular e $S$ é a cadeia a ser verificada. Um algoritmo para solução detes problema de
...
\postextualchapter{Código-fonte da Visualizador}\label{app:view_source}
