...
Uma linguagem formal é um conjunto de símbolos -- potencialmente infinito -- que pode estar restrito a um número de regras específicas a eles. Estes símbolos são construídos a partir da combinação de um conjunto finito de elementos da linguagem chamado \emph{alfabeto}. 

Por ser tratar de conjuntos, a notação clássica dos mesmos é utilizada para linguagens. E.g. $A \cup B$ representa a união de linguagens A e B, assim como $A \supset B$ é uma proposição que afirma que a linguagem A contém todos os símbolos da linguagem B. $w \in A$ afirma que o símbolo $w$ está presente na linguagem A.
...
Uma das formas de representar linguagens formais é através de gramáticas gerativas. O termo ``gramática formal'' é comumente utilizado para referir-se a gramáticas gerativas. Entretanto, existem outras formas de representar gramáticas formais além deste. Este trabalho irá se limitar à definição mais comum.

Gramáticas gerativas são sistemas de reescrita, i.e. a partir de um símbolo inicial, é possível usar as regras de reescrita para produzir todos os símbolos de uma linguagem. Esta operação é, de certa forma, o problema dual de reconhecer um símbolo como parte da linguagem através de um autômato \cite{bib:Ruohonen09}.

Uma gramática é formada pela quádrupla: $(N, \Sigma, P, S)$. Onde:
...
Um exemplo de gramática é a que define expressões da forma $a^nb^n$:
...
Perceba que todas as regras do lado esquerdo possuem apenas um símbolo não terminal. Se fôssemos escrever uma gramática para expressões na forma $a^nb^nc^n$, precisaríamos de uma gramática mais rebuscada:
...
Por exemplo, uma linguagem composta por símbolos repetidos, como \emph{DogDog} ou \emph{CatCat}. É possível definir uma expressão regular $/(.*)\backslash 1/$ que reconhece esta linguagem. o $\backslash 1$ instrui ao motor de reconhecimento a esperar um novo símbolo exatamente igual ao obtido pelo reconhecimento do primeiro grupo $(.*)$.
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\section{Construção do Autômato}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
